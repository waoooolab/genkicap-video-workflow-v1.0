  ...
  try {
    ...
    if (fs.existsSync(path.join(packageRoot, 'scripts', guideFileName))) {
      fs.copyFileSync(
        ...
        path.join(`./${scriptsDir}`, guideFileName)
      );
    }
    ...
    // Task 2: Copy .claude/
    copyDir(path.join(packageRoot, '.claude'), './.claude', []);
  ...
  try {
    ...
    for (let i = 0; i < taskMapping.length; i++) {
      ...
      if (index === 0) {
        ...
        // Update only the guide file
        if (fs.existsSync(path.join(packageRoot, 'references', guideFileName))) {
          fs.copyFileSync(
            path.join(packageRoot, 'references', guideFileName),
            path.join(refPath, guideFileName)
          );
        }
      }
