...
const { migrateWorkspace } = require('./migrate');
...
async function importNewWorkflow(lang) {
  ...
  try {
    ...
    // 中文工作空间使用中文文件名
    const readmeDest = dirLang === 'zh' ? './说明.md' : './README.md';
    const quickstartDest = dirLang === 'zh' ? './快速开始.md' : './QUICKSTART.md';

    if (fs.existsSync(path.join(packageRoot, readmeSrc))) {
      fs.copyFileSync(path.join(packageRoot, readmeSrc), readmeDest);
    }
    if (fs.existsSync(path.join(packageRoot, quickstartSrc))) {
      fs.copyFileSync(path.join(packageRoot, quickstartSrc), quickstartDest);
    }
...
async function updateWorkflow(lang) {
  ...
  // Silently migrate workspace structure if needed
  console.log('\n[Migration Debug] Starting migration...');
  console.log('  dirLang:', dirLang);
  console.log('  workspacePath:', process.cwd());

  const migrationResult = migrateWorkspace(dirLang, process.cwd());
  ...
  if (migrationResult.migrated.length > 0) {
    ...
    migrationResult.migrated.forEach(item => console.log('    -', item));
  }
  if (migrationResult.skipped.length > 0) {
    console.log('  Skipped items:');
    migrationResult.skipped.forEach(item => console.log('    -', item));
  }
  console.log('[Migration Debug] Done.\n');
...
async function deleteWorkflow(lang) {
  ...
  console.log();
  showWarning(lang === 'zh' ? '不会删除 config.json、references/ 和 scripts/' : 'Will NOT delete config.json, references/ and scripts/');
  console.log();
  ...
  try {
    ...
    // Task 2: Delete README.md, QUICKSTART.md (支持中英文文件名)
    // 英文文件名
    ...
    // 中文文件名
    if (fs.existsSync('./说明.md')) {
      fs.rmSync('./说明.md', { force: true });
    }
    if (fs.existsSync('./快速开始.md')) {
      fs.rmSync('./快速开始.md', { force: true });
    }
    await updateTaskStatus(1);
