...
async function initializationMode(lang, indent) {
  ...
  try {
    ...
    if (fs.existsSync(path.join(packageRoot, '.claude'))) {
      copyDir(path.join(packageRoot, '.claude'), path.join(targetDir, '.claude'), []);
    }
    ...
    // Copy reference guide file to workspace
    const guideFileName = '_GUIDE.md';
    if (fs.existsSync(path.join(packageRoot, 'references', guideFileName))) {
      fs.copyFileSync(
        path.join(packageRoot, 'references', guideFileName),
        path.join(referencesPath, guideFileName)
      );
    }
    ...
    // Copy scripts guide file to workspace
    if (fs.existsSync(path.join(packageRoot, 'scripts', guideFileName))) {
      fs.copyFileSync(
        path.join(packageRoot, 'scripts', guideFileName),
        path.join(scriptsPath, guideFileName)
      );
    }

    // Copy documentation files
    if (fs.existsSync(path.join(packageRoot, 'README.md'))) {
      fs.copyFileSync(path.join(packageRoot, 'README.md'), path.join(targetDir, 'README.md'));
    }
    if (fs.existsSync(path.join(packageRoot, 'QUICKSTART.md'))) {
      fs.copyFileSync(path.join(packageRoot, 'QUICKSTART.md'), path.join(targetDir, 'QUICKSTART.md'));
    }
