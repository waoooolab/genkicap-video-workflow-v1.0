...
async function initializationMode(lang, indent) {
  ...
  try {
    ...
    if (fs.existsSync(path.join(packageRoot, readmeSrc))) {
      fs.copyFileSync(path.join(packageRoot, readmeSrc), path.join(targetDir, 'README.md'));
    }
    if (fs.existsSync(path.join(packageRoot, quickstartSrc))) {
      fs.copyFileSync(path.join(packageRoot, quickstartSrc), path.join(targetDir, 'QUICKSTART.md'));
    }
