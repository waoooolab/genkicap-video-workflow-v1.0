...
// Legacy project-level files that should be deleted
const LEGACY_PROJECT_FILES = {
  zh: ['_context', '_context.md'],  // Old context file (with or without .md extension), now split into separate files
  en: ['_context', '_context.md']
};
...
function migrateWorkspace(dirLang, workspacePath = process.cwd()) {
  ...
  for (const project of projects) {
    ...
    // 3d. Delete legacy project files
    const legacyFiles = LEGACY_PROJECT_FILES[dirLang] || [];
    for (const legacyFile of legacyFiles) {
      const legacyFilePath = path.join(projectPath, legacyFile);
      if (fs.existsSync(legacyFilePath)) {
        try {
          fs.unlinkSync(legacyFilePath);
          result.migrated.push(`${project}/${legacyFile} (deleted, no longer needed)`);
        } catch (err) {
          result.skipped.push(`${project}/${legacyFile} (delete failed: ${err.message})`);
        }
      }
    }

    // 3e. Ensure contexts directory has _meta.json
    const contextsDir = path.join(projectPath, currentDirNames.contexts);
    if (fs.existsSync(contextsDir)) {
      const metaPath = path.join(contextsDir, '_meta.json');
      if (!fs.existsSync(metaPath)) {
        try {
          const metaContent = {
            description: "Supplementary materials directory",
            created: new Date().toISOString(),
            version: "1.0"
          };
          fs.writeFileSync(metaPath, JSON.stringify(metaContent, null, 2), 'utf8');
          result.migrated.push(`${project}/${currentDirNames.contexts}/_meta.json (created)`);
        } catch (err) {
          result.skipped.push(`${project}/${currentDirNames.contexts}/_meta.json (create failed: ${err.message})`);
        }
      }
    }
  }
...
function needsMigration(dirLang, workspacePath = process.cwd()) {
  ...
  for (const project of projects) {
    ...
    // Check for legacy project files that should be deleted
    const legacyFiles = LEGACY_PROJECT_FILES[dirLang] || [];
    for (const legacyFile of legacyFiles) {
      const legacyFilePath = path.join(projectPath, legacyFile);
      if (fs.existsSync(legacyFilePath)) {
        return true; // Need to delete this file
      }
    }

    // Check if contexts directory needs _meta.json
    const contextsDir = path.join(projectPath, currentDirNames.contexts);
    if (fs.existsSync(contextsDir)) {
      const metaPath = path.join(contextsDir, '_meta.json');
      if (!fs.existsSync(metaPath)) {
        return true; // Need to create _meta.json
      }
    }
  }
