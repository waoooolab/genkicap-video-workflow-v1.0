...
async function repairWorkspace(lang) {
  ...
  if (result.migrated.length > 0) {
    ...
    result.migrated.forEach(item => console.log(`${indent}  ${colors.green}✓${colors.reset} ${item}`));
    console.log();
  }

  if (result.skipped.length > 0) {
    showWarning(lang === 'zh' ? '已跳过（目标已存在）：' : 'Skipped (target exists):');
    result.skipped.forEach(item => console.log(`${indent}  ${theme.muted}○${colors.reset} ${item}`));
    console.log();
  }
