...
const { migrateWorkspace, needsMigration } = require('./migrate');
const { loadWorkspaceConfig } = require('../utils/config');
...
async function checkUpdate(lang) {
  ...
  try {
    ...
    try {
      ...
      if (!latestVersion || latestVersion.includes('404') || latestVersion.includes('Not Found') || latestVersion.includes('E404')) {
    ...
    if (currentVersion === latestVersion) {
      ...
      if (shouldUpdate) {
        console.log();
        showInfo(lang === 'zh' ? '正在更新...' : 'Updating...', indent);
        console.log();

        try {
          execSync(`npm install -g ${packageName}@latest`, { stdio: 'inherit' });
          console.log();
          showSuccess(lang === 'zh' ? '更新完成！' : 'Update complete!', indent);
          console.log();
          showInfo(lang === 'zh' ? '请重新运行命令以使用新版本' : 'Please restart to use the new version', indent);
          console.log();
          showWarning(lang === 'zh' ? '更新后请使用菜单 \"2. 导入/更新工作流\" 更新工作空间' : 'After update, please use menu \"2. Import/Update Workflow\" to update workspace', indent);
          console.log();
        } catch (err) {
          console.log();
          showError(`${lang === 'zh' ? '更新失败' : 'Update failed'}: ${err.message}`, indent);
          console.log();
          showInfo(lang === 'zh' ? '手动更新方法：' : 'Manual update method:', indent);
...
/**
 * Repair workspace - check and fix directory structure
 */
async function repairWorkspace(lang) {
  const indent = '  ';

  showSubMenuTitle(lang === 'zh' ? '修复工作空间' : 'Repair Workspace', lang, indent);

  // Check if in workspace
  const config = loadWorkspaceConfig();
  if (!config) {
    showError(lang === 'zh' ? '未找到工作空间配置' : 'Workspace config not found');
    console.log();
    showInfo(lang === 'zh' ? '请在工作空间目录中运行此命令' : 'Please run this command in a workspace directory');
    console.log();
    await askInput(lang === 'zh' ? '按 Enter 返回' : 'Press Enter to return', '', indent);
    return;
  }

  const dirLang = config.dirLang || 'zh';

  // Check if migration needed
  if (!needsMigration(dirLang)) {
    showSuccess(lang === 'zh' ? '工作空间结构正常，无需修复' : 'Workspace structure is correct, no repair needed');
    console.log();
    await askInput(lang === 'zh' ? '按 Enter 返回' : 'Press Enter to return', '', indent);
    return;
  }
  ...
  showInfo(lang === 'zh' ? '检测到需要修复的项目：' : 'Items that need repair:');
  console.log();

  // Run migration
  const result = migrateWorkspace(dirLang);

  if (result.migrated.length > 0) {
    showSuccess(lang === 'zh' ? '已修复：' : 'Repaired:');
    result.migrated.forEach(item => {
      // Translate migration messages
      let displayItem = item;
      if (lang === 'zh') {
        displayItem = displayItem
          .replace('(deleted, no longer needed)', '(已删除，不再需要)')
          .replace('(created)', '(已创建)')
          .replace('(both empty, removed old)', '(都为空，已删除旧目录)')
          .replace('(old empty, removed)', '(旧目录为空，已删除)')
          .replace('(new empty, replaced)', '(新目录为空，已替换)')
          .replace('(both have content, merged)', '(都有内容，已合并)');
      }
      console.log(`${indent}  ${colors.green}✓${colors.reset} ${displayItem}`);
    });
    console.log();
  }

  if (result.skipped.length > 0) {
    showWarning(lang === 'zh' ? '已跳过（目标已存在）：' : 'Skipped (target exists):');
    result.skipped.forEach(item => {
      // Translate skip messages
      let displayItem = item;
      if (lang === 'zh') {
        displayItem = displayItem
          .replace('(migration failed:', '(迁移失败：')
          .replace('(delete failed:', '(删除失败：')
          .replace('(create failed:', '(创建失败：')
          .replace('exists)', '已存在)');
      }
      console.log(`${indent}  ${theme.muted}○${colors.reset} ${displayItem}`);
    });
    console.log();
  }

  if (result.migrated.length === 0 && result.skipped.length === 0) {
    showSuccess(lang === 'zh' ? '无需修复' : 'No repair needed');
    console.log();
  }
