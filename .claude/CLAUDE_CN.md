...
**文件命名规范**：
- 阶段文件：`选题沟通.md`、`框架搭建.md`、`内容调研.md`、`大纲确认.md`、`脚本草稿.md`
- 归档文件：`选题沟通_v01.md`、`框架搭建_v02.md`（按修改顺序递增）
...
1. **读取选题** - 从 `{CURRENT_PROJECT}/阶段/选题沟通.md` 读取选题信息
...
1. **读取前置信息** - 从选题、大纲、调研文件获取必要信息（平台、时长、语言等）
...
**更新文件**: `{CURRENT_PROJECT}/阶段/脚本草稿.md`
...
1. **复制脚本内容** - 从草稿文件复制完整脚本
2. **添加元信息** - 标题、时长、字数、日期
3. **生成统计表格** - 各板块字数和时长统计
4. **输出到项目根目录** - 作为最终交付成果

根据**模版**: `.claude/template/stage/zh-CN/script.md`
**输出**: `{CURRENT_PROJECT}/最终脚本.md`（保留草稿文件作为备份）
...
- **Read/Write/Edit**: 读取和写入 `{CURRENT_PROJECT}/阶段/` 文件
