...
- **需求意图识别**: 从用户零散描述中提取核心信息,判断选题方向(新方向/现有账号/特定平台),理解目标受众和内容定位。**重要**：即使 config.json 配置为"多平台"、"多种时长"，也要从用户描述中识别**本项目的具体平台和时长**，记录到项目 _meta.json 的 target 字段
...
2. **识别项目参数** - 从用户描述中提取本项目的具体参数：
   - **目标平台**: 如果用户提到"抖音"、"B站"等，记录具体平台；否则询问
   - **目标时长**: 如果用户提到"3分钟"、"短视频"等，记录具体时长；否则询问
   - **目标受众**: 从选题内容推断或询问
   - 将这些参数写入 `{CURRENT_PROJECT}/_meta.json` 的 `target` 字段
3. **补充信息** - 从 config.json 读取配置作为参考，但以项目 _meta.json 中的 target 为准
4. **搜索调研** - WebSearch 或者通过 MCP工具 搜索相关内容、竞品、热门视频
5. **生成方案** - 输出结构化选题,包含:
   - 竞品参考 (≥2个)
   - 选题角度 (≥3个)
   - 标题建议 (每个角度 2-3个)
6. **用户确认** - 选择角度进入下一阶段,或提出修改要求
